...
double timeCounting(int *a, int n, int iAlgo)
{
    ...
    duration<double, std::micro> elapsedTime = duration_cast<duration<double>>(end - start);
    return elapsedTime.count() / 1E6; // Trả về thời gian tính bằng giây
}
...
void commandFive(char *givenAlgo1, char *givenAlgo2, char *inputSize, char *ipOrder)
{
    ...
    std::cout << "Input order: " << ipOrder+1 << "\n";
